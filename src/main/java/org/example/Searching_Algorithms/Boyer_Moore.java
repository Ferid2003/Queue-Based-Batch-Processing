...
public class Boyer_Moore {
    ...
    public static void search(char txt[], char pat[], BufferedWriter writer) throws IOException {
        int m = pat.length;
        int n = txt.length;

        int badchar[] = new int[NO_OF_CHARS];
        List<String> found_pattern = new ArrayList<String>();
        ...
        while (s <= (n - m)) {
			...
			the above loop */
            if (j < 0) {
                found_pattern.add("Patterns occur at shift = " + s+"\n");
//                System.out.println(
//                        "Patterns occur at shift = " + s);
        ...
        writer.write(String.valueOf(found_pattern));
    }
}
